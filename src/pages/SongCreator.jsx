  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      await base44.entities.ContentAsset.create({
        ...
        file_url: dubbedAudioUrl || audioUrl || undefined,
        ai_generated: true,
    ...
    <div className="min-h-screen bg-[#0a0a0a] p-4 md:p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-pink-600 flex items-center justify-center shadow-lg shadow-violet-500/20 shrink-0">
            <Music className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="text-xl font-black text-foreground">Song Creator</h1>
            <p className="text-xs text-muted-foreground">AI lyrics, voiceover rendering, and multilingual dubbing</p>
          </div>
        </div>
            ...
            <div className="space-y-4">
              {lyrics ? (
                <>
                  ...
                  <button onClick={() => generateAudio(lyrics, (url) => setAudioUrl(url))} disabled={voiceLoading}
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-violet-500/10 border border-violet-500/20 text-violet-400 text-sm font-semibold hover:bg-violet-500/20 transition-all disabled:opacity-50">
                    {voiceLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Mic2 className="w-4 h-4" />}
                    {voiceLoading ? "Rendering audio…" : audioUrl ? "Re-render Voiceover" : "Render AI Voiceover"}
                  </button>
                  {audioUrl && (
                    <div className="p-3 rounded-xl bg-card border border-border">
                      <p className="text-xs font-semibold text-muted-foreground mb-2">AI Voiceover — {language}</p>
              ...
              <>
                ...
                {dubbedLyrics && (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => generateAudio(dubbedLyrics, (url) => setDubbedAudioUrl(url))} disabled={voiceLoading}
